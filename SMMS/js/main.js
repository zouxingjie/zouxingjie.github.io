			...
			table = $("#datatable-buttons").DataTable({
				...
				buttons: [{
						...
						action: function (e, dt, node, config) {
							Object.keys(slopeInfo).forEach(key => slopeInfo[key] = "");
							$('#detailModal #detailModalLabel').text('ADD SLOPE');
							$('#detailModal fieldset').attr('disabled', false);
							$('#detailModal #commit').attr("data-type", "add").show();
				...
				"rowCallback": function (row, data, index) {
					...
					//format MAp clounm
					var showMap = data['MAP'] == "" ? "" : "<a href='" + data['MAP'] + "' target='_blank'>show</a>";
					$('td', row).eq(2).html(showMap);
					//format PHOTO clounm
					var showPhoto = data['PHOTO'] == "" ? "" : "<a href='#' data-imgUrls = " + data['PHOTO'] + " onclick='showPhoto(this);'>show</a>";
...
function showPhoto(elem) {
	var imgUrls = $(elem).attr("data-imgUrls").split(",");
	var modalContent = ""
	for (var i = 0, len = imgUrls.length; i < len; i++) {
		modalContent += '<a href="' + imgUrls[i] + '" data-lightbox="group"><img width="200" src="' + imgUrls[i] + '" alt=""></a>';
	}
	$("#photoModal div.modal-body").empty().html(modalContent);
